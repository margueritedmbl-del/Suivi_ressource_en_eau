...
export async function buildInstitutionalDashboard(): Promise<InstitutionalDashboardData> {
  ...
  const modules = {
    points_eau: { label: "Points d’eau", ...peDashboard, stats: { ...peDashboard.stats, sites: peDashboard.stats.total, observations: peDashboard.stats.total, alertes: peDashboard.stats.alertes_qualite ?? peDashboard.stats.non_fonctionnels ?? 0 } },
  ...
  const stats = {
    ...
    points_donnees_a_verifier: peDashboard.stats.donnees_a_verifier,
  ...
  const charts = {
    modules: [
      { label: "Pluie", value: pluvioSummary.couverture_pct },
    ...
    couverture_territoriale: [
      ...
      { label: "Stations pluie", value: pluvioSummary.sites },
