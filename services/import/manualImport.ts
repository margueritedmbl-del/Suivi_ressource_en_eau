...
async function upsertBatchAdaptive(
  ...
  // Une ligne invalide ne doit pas faire échouer les centaines d'autres.
  if (rows.length > 1) {
  ...
  // Repli sur une clé métier unique si le conflit ne porte pas sur source_entry_id.
...
export async function importManualFile(args: {
  ...
  let failed = 0;
  // Petits lots : rapides sur Vercel et suffisamment fins pour isoler les lignes invalides.
  const batchSize = 50;

  for (let offset = 0; offset < mappedRows.length; offset += batchSize) {
    const batch = mappedRows.slice(offset, offset + batchSize);
    const outcome = await upsertBatchAdaptive(target, batch, offset, errors);
