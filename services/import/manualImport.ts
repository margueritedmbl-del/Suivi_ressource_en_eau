...
/**
 * Convertit les dates françaises/Excel vers le format ISO attendu par PostgreSQL.
 * Formats acceptés notamment : 22/07/2025, 22-07-2025, 2025/07/22,
 * 22/07/2025 08:30 et les objets Date issus d'Excel.
 */
export function normalizeImportDate(value: unknown): string | null {
  ...
  // Numéro de série Excel (protection supplémentaire si un classeur est lu en mode raw).
  if (typeof value === "number" && Number.isFinite(value) && value > 0) {
    const decoded = XLSX.SSF.parse_date_code(value);
    if (decoded && validIsoDate(decoded.y, decoded.m, decoded.d)) {
  ...
  // Formats français jour/mois/année, jour-mois-année ou jour.mois.année,
  // avec heure facultative après la date.
  ...
  for (const [key, value] of Object.entries(input || {})) {
    ...
    if (DATE_ONLY_FIELDS.has(key)) {
      ...
      if (value !== null && value !== "" && normalized === null) {
        dateWarnings.push(`${key}="${String(value).slice(0, 80)}"`);
      }
      continue;
    }
    output[key] = value;
  }
  return { row: output, dateWarnings };
}
...
export function mapManualRows(targetKey: string, rows: Record<string, any>[], baseIndex = 0): MappedChunk {
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    try {
      const cleaned = cleanMappedRow(target.mapper(source));
      const mapped = cleaned.row;
      if (cleaned.dateWarnings.length && warnings.length < 30) {
        warnings.push(`Ligne ${lineNumber} : date(s) non reconnue(s) remplacée(s) par une valeur vide (${cleaned.dateWarnings.join(", ")}).`);
      }
