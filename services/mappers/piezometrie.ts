...
export function mapPiezometre(entry:any){
  const loc=findLocation(entry);const commune=pick(entry,["Commune","Nom de la Commune"]);const village=pick(entry,["Villages","Village","Localité","Localite","Nom de la localité / site","Site","Nom du site"]);const rawCode=pick(entry,["5_Code_pizomtre","Code piézomètre","Code piezometre","Code du piézomètre","Code / Numéro du piézomètre"]);const ref=findPiezoReference([village,rawCode]);
