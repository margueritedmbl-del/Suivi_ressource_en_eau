import { getValue, getValueExact, findLocation, safeNumber, safeStrictNumber, safeDate, sourceId, rawEntry } from "../epicollect/client";

function val(entry: any, labels: string[]) { return getValue(entry, labels); }
function dateValue(entry: any, labels: string[]) { return safeDate(val(entry, labels)); }
function exactMeasure(entry:any,labels:string[]){return safeStrictNumber(getValueExact(entry,labels));}
...
export function mapPointEau(entry: any) {
  ...
  return {
    code_pe: val(entry, ["Code PE", "Code point d'eau", "Code du point d'eau", "code_pe"]) || `PE-${String(id).replace(/[^a-zA-Z0-9]/g, "").slice(0, 12)}`,
    type_ouvrage: type,
    titre_source: title,
    ...
    temperature_c: exactMeasure(entry, ["39_Temprature_C", "Température C", "Temperature C"]),
    ph: exactMeasure(entry, ["40_pH_", "pH"]),
    conductivite: exactMeasure(entry, ["41_Conductivit_lectr", "Conductivité électrique", "Conductivite"]),
    turbidite_ntu: exactMeasure(entry, ["42_Turbidit_NTU", "Turbidité NTU", "Turbidite"]),
    tds: exactMeasure(entry, ["43_TDS_", "TDS"]),
    ...
    created_at_source: entry?.created_at || null,
    uploaded_at_source: entry?.uploaded_at || null,
