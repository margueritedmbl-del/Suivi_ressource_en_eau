...
async function fetchWithRetry(requestUrl:string,options:Required<Pick<EpicollectFetchOptions,"pageDelayMs"|"maxRetries"|"baseRetryDelayMs"|"maxRetryDelayMs">>,stats:PageFetchStats){
  for(let attempt=0;attempt<=options.maxRetries;attempt++){
    ...
    const response=await fetch(requestUrl,{cache:"no-store",headers:{Accept:"application/json","User-Agent":"PSORE-Epicollect-Synchronizer/4.5.0"}});
...
export function parentSourceId(e:any){return e?.ec5_parent_uuid||e?.parent_uuid||e?.parent?.data?.parent_uuid||e?.parent?.parent_uuid||e?.parent?.uuid||null}
