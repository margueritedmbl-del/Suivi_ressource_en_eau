...
async function fetchWithRetry(requestUrl:string,options:Required<Pick<EpicollectFetchOptions,"pageDelayMs"|"maxRetries"|"baseRetryDelayMs"|"maxRetryDelayMs">>,stats:PageFetchStats){
  for(let attempt=0;attempt<=options.maxRetries;attempt++){
    ...
    const response=await fetch(requestUrl,{cache:"no-store",headers:{Accept:"application/json","User-Agent":"PSORE-Epicollect-Synchronizer/4.7.0"}});
...
export function getValue(e:any,labels:string[]){const wanted=labels.map(norm).filter(Boolean);let res:any=null;const matches=(key:string)=>{const nk=norm(key);const clean=nk.replace(/^\d+/,"");return wanted.some(w=>nk===w||clean===w||(clean.length>=8&&(w.startsWith(clean)||clean.startsWith(w)))||(w.length>=8&&nk.endsWith(w)));};function scan(v:any){if(res!==null||!v||typeof v!=="object")return;if(Array.isArray(v)){v.forEach(scan);return}for(const[k,val]of Object.entries(v)){if(matches(k)){res=unwrapAnswer(val);return}scan(val)}}scan(e);return res}
...
export function getValueExact(e:any,labels:string[]){const wanted=new Set(labels.map(norm).filter(Boolean));let res:any=null;function scan(v:any){if(res!==null||!v||typeof v!=="object")return;if(Array.isArray(v)){for(const x of v){scan(x);if(res!==null)return}return}for(const[k,val]of Object.entries(v)){if(wanted.has(norm(k))){res=unwrapAnswer(val);return}}for(const val of Object.values(v)){scan(val);if(res!==null)return}}scan(e);return res}
export function safeStrictNumber(v:any){if(v===null||v===undefined||v==="")return null;if(typeof v==="number")return Number.isFinite(v)?v:null;const s=String(unwrapAnswer(v)??"").trim().replace(",",".");if(!/^-?\d+(?:\.\d+)?$/.test(s))return null;const n=Number(s);return Number.isFinite(n)?n:null}
...
export function findLocation(e:any){let loc=parseLocation(getValue(e,["Coordonnées GPS","Coordonnees GPS","Coordonnées station","Coordonnees station","Coordonnées géographiques","Coordonnees geographiques","Coordonnées infrastructures","Coordonnees infrastructures","10_Coordonnes_infras","10_Coordonnes_GPS","9_Cordonnes_gographi","10_Coordonnes_statio","GPS","Geolocalisation","Géolocalisation","Localisation GPS","Point GPS","location","coordinates","Latitude Longitude"]));if(loc.latitude!==null&&loc.longitude!==null)return loc;loc={latitude:safeNumber(getValue(e,["Latitude","lat","Y","lat_10_Coordonnes_infras","lat_10_Coordonnes_GPS","lat_9_Cordonnes_gographi","lat_10_Coordonnes_statio"])),longitude:safeNumber(getValue(e,["Longitude","long","lng","lon","X","long_10_Coordonnes_infras","long_10_Coordonnes_GPS","long_9_Cordonnes_gographi","long_10_Coordonnes_statio"]))};if(loc.latitude!==null&&loc.longitude!==null)return loc;let found:any={latitude:null,longitude:null};function walk(v:any){if(found.latitude!==null&&found.longitude!==null)return;if(!v||typeof v!=="object")return;if(!Array.isArray(v)){let lat:any=null,lon:any=null;for(const[k,val]of Object.entries(v)){const nk=norm(k);if(lat===null&&nk.startsWith("lat"))lat=safeNumber(val);if(lon===null&&(nk.startsWith("long")||nk.startsWith("lng")||nk.startsWith("lon")))lon=safeNumber(val);}if(lat!==null&&lon!==null){found={latitude:lat,longitude:lon};return}}const p=parseLocation(v);if(p.latitude!==null&&p.longitude!==null){found=p;return}Array.isArray(v)?v.forEach(walk):Object.values(v).forEach(walk)}walk(e);return found}
