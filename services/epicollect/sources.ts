...
export async function getActiveEpicollectSources(module?: string): Promise<EpicollectSource[]> {
  try {
    ...
    if (data?.length) {
      ...
      // V4.7 : la configuration embarquée corrige les trois formulaires enfants même si la table
      ...
      return rows.map((row)=>{
        ...
        const isChild=["pluviometrie:releves","piezometrie:mesures","limnimetrie:lectures"].includes(`${row.module}:${row.type_source}`);
        return isChild&&canonical?{...row,api_url:canonical.api_url}:row;
      });
    }
  } catch (_) {
