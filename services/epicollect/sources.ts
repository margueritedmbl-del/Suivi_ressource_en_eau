...
export function withChildParentRef(source: EpicollectSource, moduleSources: EpicollectSource[]) {
  const parentType = PARENT_SOURCE_BY_CHILD[`${source.module}:${source.type_source}`];
  if (!parentType) return source;
  const parent = moduleSources.find((s) => s.type_source === parentType);
  if (!parent) return source;
  const parentRef = formRefFromApiUrl(parent.api_url);
  if (!parentRef) return source;
  try {
    const url = new URL(source.api_url);
    if (!url.searchParams.get("branch_ref")) url.searchParams.set("parent_form_ref", parentRef);
    return { ...source, api_url: url.toString() };
  } catch { return source; }
}
