...
export type SyncResult = {
  ...
  mode?:"full"|"incremental"; filterFrom?:string|null; totalReported?:number|null; linked?:number; error?:string; details?:string[];
};

const ENGINE_REVISION="v4.5.0";
...
async function enrichFromParent(table:string,rows:any[]){const parentIds=Array.from(new Set(rows.map(r=>r.source_parent_id).filter(Boolean)));if(!parentIds.length)return{rows,linked:0};let parentTable="",idField="",codeField="";if(table==="observations_pluvio"){parentTable="stations_pluvio";idField="station_id";codeField="code_station";}else if(table==="observations_piezo"){parentTable="piezometres";idField="piezometre_id";codeField="code_piezo";}else if(table==="observations_limni"){parentTable="stations_limni";idField="station_id";codeField="code_station";}else return{rows,linked:0};
  try{const{data,error}=await supabaseAdmin.from(parentTable).select(`id,source_entry_id,${codeField}`).in("source_entry_id",parentIds);if(error||!data)return{rows,linked:0};const map=new Map((data as any[]).map(p=>[p.source_entry_id,p]));let linked=0;for(const row of rows){const p=map.get(row.source_parent_id);if(p){row[idField]=p.id;if(!row[codeField])row[codeField]=p[codeField];linked++;}}return{rows,linked};}catch{return{rows,linked:0};}
}

async function upsertResilient(table:string,rows:any[]){let upserted=0;const errors:string[]=[];async function write(batch:any[]):Promise<void>{if(!batch.length)return;const{error}=await supabaseAdmin.from(table).upsert(batch,{onConflict:"source_entry_id"});if(!error){upserted+=batch.length;return;}const msg=error.message||String(error);if(/column .* does not exist|schema cache|could not find/i.test(msg))throw new Error(`${msg}. Exécutez la migration database/19_sync_child_forms_v4_5.sql avant de synchroniser.`);if(batch.length===1){errors.push(`${batch[0]?.source_entry_id||"ligne"}: ${msg}`);return;}const mid=Math.ceil(batch.length/2);await write(batch.slice(0,mid));await write(batch.slice(mid));}
...
export async function syncTable(source:EpicollectSource,table:string,mapper:(e:any)=>any,options:{full?:boolean}={}):Promise<SyncResult>{
  ...
  try{
    ...
    const enriched=await enrichFromParent(table,valid);const written=await upsertResilient(table,enriched.rows);const skipped=fetched.entries.length-valid.length+written.errors.length;
    const result:SyncResult={...base,status:"success",fetched:fetched.entries.length,mapped:mapped.length,skipped,rejected:rejects.length+written.errors.length,upserted:written.upserted,pages:fetched.pages,durationMs:Date.now()-started,mode,filterFrom:lastSync,totalReported:fetched.totalReported,linked:enriched.linked,details:written.errors.slice(0,20)};
    const note=`engine=${ENGINE_REVISION}; ${source.libelle}: ${written.upserted}/${fetched.entries.length} intégré(s), ${rejects.length} donnée(s) non exploitable(s), ${written.errors.length} rejet(s) SQL, liés parent=${enriched.linked}, mode=${mode}, pages=${fetched.pages}, per_page=${fetched.perPage}, total_api=${fetched.totalReported??"?"}, retries=${fetched.retries}.`;
