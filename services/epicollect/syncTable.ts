...
async function upsertResilient(table:string,rows:any[]){let upserted=0;const errors:string[]=[];async function write(batch:any[]):Promise<void>{if(!batch.length)return;const{error}=await supabaseAdmin.from(table).upsert(batch,{onConflict:"source_entry_id"});if(!error){upserted+=batch.length;return;}const msg=error.message||String(error);if(/column .* does not exist|schema cache|could not find/i.test(msg))throw new Error(`${msg}. Exécutez la migration database/21_MASTER_REPAIR_V4_6.sql avant de synchroniser.`);if(batch.length===1){errors.push(`${batch[0]?.source_entry_id||"ligne"}: ${msg}`);return;}const mid=Math.ceil(batch.length/2);await write(batch.slice(0,mid));await write(batch.slice(mid));}
...
export async function syncTable(source:EpicollectSource,table:string,mapper:(e:any)=>any,options:{full?:boolean}={}):Promise<SyncResult>{
  ...
  try{
    ...
    const result:SyncResult={...base,status:"success",fetched:fetched.entries.length,mapped:mapped.length,skipped,rejected:rejects.length+written.errors.length,upserted:written.upserted,pages:fetched.pages,durationMs:Date.now()-started,mode,filterFrom:lastSync,totalReported:fetched.totalReported,linked:enriched.linked,details:[...(written.errors.slice(0,20)),...(reconciledDeleted?[`Réconciliation : ${reconciledDeleted} ancienne(s) ligne(s) supprimée(s)`]:[])]};
    const note=`engine=${ENGINE_REVISION}; ${source.libelle}: ${written.upserted}/${fetched.entries.length} intégré(s), ${rejects.length} donnée(s) non exploitable(s), ${written.errors.length} rejet(s) SQL, liés parent=${enriched.linked}, mode=${mode}, pages=${fetched.pages}, per_page=${fetched.perPage}, total_api=${fetched.totalReported??"?"}, retries=${fetched.retries}, anciennes_supprimees=${reconciledDeleted}.`;
