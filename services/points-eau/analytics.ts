...
function explicitInterventionNeed(value:string){
  const v=normalizedLower(value);
  if(!v||["ras","aucun","aucune","neant","non","rien","non renseigne","repare","reparee","rehabilite","rehabilitee"].includes(v))return false;
  return /(rehabil|repar|entretien|amelior|clotur|securis|pompe|flotteur|couvercle|diagnostic|analyse|extension|formation|cylindre|murette|ouvrage)/.test(v);
}
...
export function normalizePointEau(row: PointEauRow): PointEauRow {
  ...
  if (explicitInterventionNeed(besoin)) score += 3;
  if (hasProblem) score += 2;
  const phPhysicallyValid = ph !== null && ph >= 0 && ph <= 14;
  const phDataInvalid = ph !== null && !phPhysicallyValid;
  if (phPhysicallyValid && (ph < 6.5 || ph > 8.5)) score += 2;
  ...
  return {
    ...
    alerte_ph: phPhysicallyValid && (ph < 6.5 || ph > 8.5),
    alerte_ph_donnee: phDataInvalid,
    alerte_odeur: normalizedLower(odeur).includes("oui"),
    alerte_photo: !asText(pick(row, "photo_infrastructure", "11_Photo_Infrastruct")),
    alerte_qualite_eau: (phPhysicallyValid && (ph < 6.5 || ph > 8.5)) || (temperature !== null && temperature > 50) || normalizedLower(odeur).includes("oui"),
  };
}
...
export function buildPointEauDashboard(rows: PointEauRow[]) {
  ...
  const aRehabiliter = rows.filter((r) => explicitInterventionNeed(r.besoin_rehabilitation) && ["non fonctionnel","abandonne","fonctionnalite partielle"].some(x=>normalizedLower(r.statut_fonctionnalite).includes(x))).length;
  const sansGps = rows.filter((r) => r.alerte_gps).length;
  const alertesQualite = rows.filter((r) => r.alerte_qualite_eau).length;
  const donneesAVerifier = rows.filter((r) => r.alerte_gps || r.alerte_ph_donnee).length;
  ...
  return {
    stats: {
      ...
      donnees_a_verifier: donneesAVerifier,
      ph_moyen: (()=>{const xs=rows.map(r=>asNumber(r.ph)).filter((n): n is number=>n!==null&&n>=0&&n<=14);return xs.length?Math.round(xs.reduce((a,b)=>a+b,0)/xs.length*100)/100:null})(),
    ...
    charts: {
      ...
      qualite_donnees: [
        ...
        { label: "Données à vérifier", value: donneesAVerifier },
      ],
    },
    ...
    controles: {
      ...
      ph_donnees_invalides: rows.filter((r) => r.alerte_ph_donnee).slice(0, 50),
