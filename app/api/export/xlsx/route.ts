...
const TABLES: Record<string, string[]> = {
  pluviometrie: ["v_pluviometrie_dashboard_v47", "observations_pluvio"],
  piezometrie: ["v_piezometrie_dashboard_v47", "observations_piezo"],
  limnimetrie: ["v_limnimetrie_dashboard_v47", "observations_limni"],
...
async function readRows(module: string) {
  if (["pluviometrie","piezometrie","limnimetrie"].includes(module)) return (await loadHydroRows(module as any)).rows;
...
export async function GET(req: NextRequest) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, module.slice(0, 31));
  const buffer = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });
  return new NextResponse(buffer, { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Content-Disposition": `attachment; filename="${module}_PSORE_V2_4.xlsx"` } });
}
