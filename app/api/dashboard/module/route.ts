...
async function readRows(moduleName: HydroModule, cfg: Cfg) {
  ...
  return { rows: loaded.rows as Row[], source: loaded.source, rejected: loaded.rejected || 0 };
}
...
export async function GET(req: NextRequest) {
  ...
  const { rows: rawRows, source, rejected } = await readRows(moduleName as HydroModule, cfg);
