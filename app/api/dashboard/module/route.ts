...
const CFG: Record<string, Cfg> = {
  pluviometrie: { view: "v_pluviometrie_dashboard_v451", table: "observations_pluvio", label: "Pluviométrie", valueKey: "pluie_24h_mm", dateKey: "date_observation", module: "pluviometrie", stationKey: "code_site" },
  piezometrie: { view: "v_piezometrie_dashboard_v451", table: "observations_piezo", label: "Piézométrie", valueKey: "niveau_statique", dateKey: "date_observation", module: "piezometrie", stationKey: "code_site" },
  limnimetrie: { view: "v_limnimetrie_dashboard_v451", table: "observations_limni", label: "Limnimétrie", valueKey: "hauteur_eau", dateKey: "date_observation", module: "limnimetrie", stationKey: "code_site" },
};
...
async function readRows(cfg: Cfg) {
  ...
  if (res.error) {
    const fallback = await supabaseAdmin.from(cfg.table).select("*").limit(20000);
    return { rows: (fallback.data || []) as Row[], source: fallback.error ? `Supabase (${fallback.error.message})` : `Supabase — table brute (vue indisponible: ${res.error.message})` };
  }
  return { rows: (res.data || []) as Row[], source: "Supabase — données validées" };
}
