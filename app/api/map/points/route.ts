...
import { networkTotal, officialCodeStrict, stationMetaByCode, type HydroModule } from "@/lib/network-registry";
import { loadOfficialStations } from "@/lib/hydro-data";
...
async function pointsEauTotal(){if(hasSupabaseAdminEnv()){const r=await supabaseAdmin.from("points_eau").select("id",{count:"exact",head:true});if(!r.error&&typeof r.count==="number")return r.count}return(pointsEauInventaire as any[]).length}

async function hydroStationRows() {
  const modules: HydroModule[] = ["pluviometrie", "piezometrie", "limnimetrie"];
  const groups = await Promise.all(modules.map(async (module) => {
    const rows = await loadOfficialStations(module);
    return rows.map((r: any) => ({
      module,
      code: r.code_site || r.code_station || r.code_piezo,
      libelle: r.nom_site || r.village || r.localite || r.cours_eau || r.code_site,
      latitude: num(r.latitude),
      longitude: num(r.longitude),
      commune: r.commune || null,
      village: r.village || null,
      couleur: moduleColors[module],
    }));
  }));
  return groups.flat();
}
...
function publicPopup(r: any) {
  ...
  return `<strong>${esc(label)}</strong><br/>Code : <b>${esc(r.code || "--")}</b><br/>Emplacement : ${esc(r.libelle || "--")}<br/>Commune : ${esc(r.commune || "--")}`;
}
...
export async function GET(req: NextRequest) {
  ...
  const modulesParam = req.nextUrl.searchParams.get("modules") ?? req.nextUrl.searchParams.get("module") ?? "none";
  ...
  const needPe = modules.includes("points_eau");
  if (needPe) {
  ...
  const peAll = await localPeRows();
  const peTotal = await pointsEauTotal();
  const peCarto = peAll.filter(valid).length;
  const summary: Record<string, number> = {
    points_eau: peTotal || peAll.length || rows.filter((r:any)=>r.module==="points_eau").length,
    pluviometrie: networkTotal("pluviometrie"),
    piezometrie: networkTotal("piezometrie"),
    limnimetrie: networkTotal("limnimetrie")
  };
  const cartographiables: Record<string, number> = {
    points_eau: peCarto || rows.filter((r:any)=>r.module==="points_eau"&&valid(r)).length,
    pluviometrie: new Set(rows.filter((r:any)=>r.module==="pluviometrie"&&valid(r)&&officialCodeStrict("pluviometrie",r.code)).map((r:any)=>officialCodeStrict("pluviometrie",r.code))).size,
    piezometrie: new Set(rows.filter((r:any)=>r.module==="piezometrie"&&valid(r)&&officialCodeStrict("piezometrie",r.code)).map((r:any)=>officialCodeStrict("piezometrie",r.code))).size,
    limnimetrie: new Set(rows.filter((r:any)=>r.module==="limnimetrie"&&valid(r)&&officialCodeStrict("limnimetrie",r.code)).map((r:any)=>officialCodeStrict("limnimetrie",r.code))).size
  };
  const data = rows
    .filter((p: any) => modules.includes(p.module))
    .map((r: any) => {
      const base={module:r.module,code:r.code,libelle:r.libelle,latitude:r.latitude,longitude:r.longitude,commune:r.commune||null,village:r.village||null,type_infrastructure:r.type_infrastructure||null,couleur:r.module === "points_eau" ? colorPoint(r, theme) : (moduleColors[r.module] || r.couleur)};
      if(detail!=="connected") return {...base,popup_html:publicPopup(r)};
      return {...r,...base,popup_html:r.module === "points_eau" ? connectedPePopup(r) : publicPopup(r)};
    });

  return NextResponse.json({ ok: true, data, summary, cartographiables, source: rows.length ? source : "aucune donnée", theme, definition:"summary=réseau physique officiel; cartographiables=stations/ouvrages avec GPS valide" });
}
