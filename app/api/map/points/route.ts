...
import { networkTotal, officialCode, stationMeta, type HydroModule } from "@/lib/network-registry";
...
async function supabaseRows() {
  ...
  let { data: view } = await supabaseAdmin.from("v_carte_points_v50").select("*").limit(20000);
  if (!view?.length) { const legacy=await supabaseAdmin.from("v_carte_points_v47").select("*").limit(20000); view=legacy.data; }
...
export async function GET(req: NextRequest) {
  ...
  const peAll = needPe ? (await localPeRows()) : [];
  const peTotal = needPe ? await pointsEauTotal() : 0;
  ...
  const cartographiables: Record<string, number> = {
    ...
    pluviometrie: rows.filter((r:any)=>r.module==="pluviometrie"&&valid(r)&&officialCode("pluviometrie",r.code,r.libelle)).length,
    piezometrie: rows.filter((r:any)=>r.module==="piezometrie"&&valid(r)&&officialCode("piezometrie",r.code,r.libelle)).length,
    limnimetrie: rows.filter((r:any)=>r.module==="limnimetrie"&&valid(r)&&officialCode("limnimetrie",r.code,r.libelle)).length
  };
  const data = rows
