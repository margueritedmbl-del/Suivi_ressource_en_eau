...
import { networkTotal, resolveStation, type HydroModule } from "@/lib/network-registry";
...
function enrichWithMeta(module: HydroModule, row: HydroRow, rawCode: any, parent?: HydroRow | null) {
  const p = parent || {};
  const locality = p.village || p.localite || p.nom_station || p.cours_eau || row.village || row.localite || row.nom_site;
  const resolution = resolveStation(module, { code: rawCode, locality });
  ...
  return {
    ...
    village: meta.locality || row.village || p.village || null,
    latitude,
    longitude,
    __station_officielle: true,
    __resolution: resolution.confidence,
  };
}
...
export async function loadHydroRows(module: HydroModule) {
  if (!hasSupabaseAdminEnv()) return { rows: [] as HydroRow[], historicalRows: [] as HydroRow[], source: "En attente configuration Supabase", rejected: 0, network: networkTotal(module), cutoff: "" };
  const cutoff = await operationalCutoff();
  // V5.2 : les tables brutes + relation parent/enfant sont la source canonique. Les vues restent un secours.
  const [raw, view] = await Promise.all([loadFromRaw(module), loadFromView(module)]);
  let historicalRows = raw.rows;
  let rejected = raw.rejected || 0;
  let source = "Supabase — parents/enfants rapprochés du référentiel canonique V5.2";
  if (!historicalRows.length && raw.error) {
    historicalRows = view.rows;
    rejected = view.rejected || 0;
    source = `Supabase — ${HYDRO_CFG[module].view} (secours)`;
  }
  const today = new Date().toISOString().slice(0,10);
  const futureRows = historicalRows.filter(r => {
    const d = dateText(r[HYDRO_CFG[module].dateField]);
    return !!d && d > today;
  });
  ...
  return { rows, historicalRows, futureRows, source, rejected, network: networkTotal(module), cutoff };
}
...
export async function loadOfficialStations(module: HydroModule) {
  ...
  for (const r of data || []) {
    const meta = resolveStation(module,{code:r[cfg.codeField],locality:r.village||r.localite||r.nom_station||r.cours_eau}).meta;
    if (!meta) continue;
