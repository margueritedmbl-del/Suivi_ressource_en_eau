...
export default function HydroTrend() {
  ...
  useEffect(() => { let alive=true; (async()=>{ try { const r=await fetch("/api/dashboard/global", { cache: "no-store" }); if(!r.ok) throw new Error(`HTTP ${r.status}`); const j=await r.json(); if(alive) setData(j?.data || {}); } catch(e:any) { if(alive){ setError(e?.message || "indisponible"); setData({}); } } })(); return()=>{alive=false}; }, []);
  ...
  return <div>{error && <div className="notice-empty">Tendances temporairement indisponibles : {error}</div>}
