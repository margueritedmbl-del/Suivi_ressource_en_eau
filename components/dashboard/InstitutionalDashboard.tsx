  ...
  return (
    <div className="institutional-dashboard">
      ...
      <div className="grid-4" style={{ marginTop: 18 }}>
        ...
        <Kpi label="Non fonctionnels" value={fmt(stats.points_non_fonctionnels)} hint={`Interventions explicites : ${fmt(stats.points_a_rehabiliter)}`} />
        <Kpi label="Données à vérifier" value={fmt(stats.points_donnees_a_verifier)} hint={`Sans GPS : ${fmt(stats.points_sans_gps)}`} />
        <Kpi label="Communes du projet" value={fmt(stats.communes_couvertes)} hint={`Dernière donnée : ${date(stats.derniere_observation)}`} />
      </div>

      <div className="grid-3" style={{ marginTop: 18 }}>
        <MiniBarChart title="Couverture du réseau (%)" data={data.charts.modules || []} />
