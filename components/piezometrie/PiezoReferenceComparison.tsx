...
function numeric(v: any) { const n = Number(String(v ?? "").replace(",", ".")); return Number.isFinite(n) ? n : null; }
...
export default function PiezoReferenceComparison() {
  ...
  const comparisons = useMemo(() => PIEZO_REFERENCES.map((ref) => {
    ...
    const candidates = rows.filter((r) => {
      ...
      if (code === ref.code.toUpperCase()) return true;
    ...
    return { ref, latest, current, ...evolution };
  }), [rows]);
  ...
  return <section className="panel piezo-reference-panel">
    <div className="panel-title-row">
      <div>
        ...
        <p>Les niveaux issus des essais d’avril–mai 2025 servent de référence. Les observations Epicollect5 sont rapprochées d’abord par code, puis par nom de localité normalisé.</p>
      </div>
    ...
    <div className="table-wrap"><table className="table"><thead><tr><th>Code</th><th>Localité</th><th>Commune</th><th>Date référence</th><th>NS référence</th><th>Dernière mesure</th><th>NS actuel</th><th>Écart</th><th>Évolution</th></tr></thead><tbody>
      {comparisons.map(({ ref, latest, current, delta, status }) => <tr key={ref.code}>
        <td><strong>{ref.code}</strong></td><td>{ref.village}</td><td>{ref.commune}</td><td>{ref.date_essai || "—"}</td><td>{fmt(ref.niveau_statique_m)} m</td><td>{latest ? dateValue(latest) || "—" : "—"}</td><td>{fmt(current)}{current === null ? "" : " m"}</td><td>{delta === null ? "—" : `${delta > 0 ? "+" : ""}${fmt(delta)} m`}</td>
