...
export default function OuvragesIntegratedDashboard(){
  const [data,setData]=useState<any>(null); const [tab,setTab]=useState<"forages"|"piezos"|"qualite">("forages"); const [filter,setFilter]=useState("Tous");
  useEffect(()=>{ fetch("/api/ouvrages/reference").then(r=>r.json()).then(j=>setData(j)).catch(()=>setData({ok:false})); },[]);
  ...
  if(!data) return <div className="panel">Chargement du référentiel intégré…</div>;
  if(!data.ok) return <div className="panel notice-empty">Impossible de charger le référentiel local.</div>;
  const r=data.resume;
  return <div className="integrated-observatory">
    ...
    {tab==="piezos" && <section className="panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>Code</th><th>Commune / site</th><th>Point</th><th>Profondeur</th><th>Niveau statique</th><th>Débit développement</th><th>Rabattement max.</th><th>Analyse eau</th><th>Documents</th></tr></thead><tbody>{piezos.map(p=><tr key={p.code}><td><strong>{p.code}</strong></td><td>{p.commune} / {p.village}</td><td>{p.point_implantation}</td><td>{fmt(p.profondeur_totale_m)} m</td><td>{fmt(p.niveau_statique_m)} m</td><td>{fmt(p.debit_developpement_m3h)} m³/h</td><td>{fmt(p.rabattement_max_m)} m</td><td><span className="tag ok">Disponible</span></td><td><a className="btn btn-soft btn-mini" href={`/documents/piezometres/essais-pompage/${p.code}.pdf`} target="_blank">Essai</a> <a className="btn btn-soft btn-mini" href={`/documents/piezometres/analyses-eau/${p.code}.pdf`} target="_blank">Analyse</a></td></tr>)}</tbody></table></div>
    </section>}

    {tab==="qualite" && <section className="panel">
      <div className="panel-title-row"><div><h2>Disponibilité des analyses d’eau</h2><p>Les 20 certificats sont recensés. Les paramètres analytiques doivent être saisis ou validés avant exploitation statistique.</p></div><a className="btn btn-soft" href="/data/referentiels/analyses_eau_piezometres_manifest.json" download><Download size={16}/> Manifest</a></div>
      <div className="quality-grid">{(data.analyses||[]).map((a:any)=><article key={a.code}><TestTube2/><div><strong>{a.code} · {a.site}</strong><span>{a.commune}</span><small>{a.statut_extraction}</small><a className="btn btn-soft btn-mini" href={a.url_certificat} target="_blank">Ouvrir le certificat</a></div></article>)}</div>
    </section>}

    <section className="panel integrated-next"><h2>Suivi opérationnel préparé</h2><p>La migration SQL fournie crée les tables nécessaires aux compteurs, volumes prélevés, niveaux statiques/dynamiques, incidents, qualité d’eau et documents techniques. Les référentiels statiques restent consultables même avant l’exécution de la migration.</p></section>
  </div>;
}
