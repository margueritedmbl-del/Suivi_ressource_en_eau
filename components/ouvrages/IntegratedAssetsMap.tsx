...
export default function IntegratedAssetsMap(){
  ...
  useEffect(()=>{let cancelled=false;(async()=>{
    ...
    barrages.filter((b:any)=>Number.isFinite(Number(b.latitude))&&Number.isFinite(Number(b.longitude))).forEach((b:any)=>L.circleMarker([Number(b.latitude),Number(b.longitude)],{radius:10,color:"#fff",weight:2,fillColor:"#166534",fillOpacity:.98}).bindPopup(`<strong>${b.code} · Micro-barrage de ${b.nom}</strong><br/>${b.commune}<br/>Travaux : ${b.statut_travaux}<br/>Surface inondée : ${b.surface_inondee_ha} ha<br/>Curage : ${b.curage_surface_ha} ha<br/>Coût APD : ${Number(b.cout_rehabilitation_fcfa).toLocaleString('fr-FR')} FCFA`).addTo(mb));
  ...
  })().catch(e=>setStatus(`Carte indisponible : ${e?.message||"erreur inconnue"}`));return()=>{cancelled=true;if(mapRef.current){mapRef.current.remove();mapRef.current=null;}}},[]);
