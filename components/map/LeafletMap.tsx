...
type BoundaryKey = "bassins" | "sousBassins" | "communes" | "restaurations";
...
const layerLabels: Record<BoundaryKey, string> = { bassins: "Bassins", sousBassins: "Sous-bassins", communes: "Communes du projet Enabel", restaurations: "Sites de restauration" };
const layerUrls: Record<BoundaryKey, string> = { bassins: "/data/hydrographie/grand_bassin.geojson", sousBassins: "/data/hydrographie/sous_bassins.geojson", communes: "/data/decisionnel/communes_projet_enabel.geojson", restaurations: "/data/decisionnel/restaurations.geojson" };
const defaultStyles: Record<BoundaryKey, LayerStyle> = {
  ...
  communes: { stroke: "#475569", fill: "#cbd5e1", weight: 1.7, opacity: 0.85, fillOpacity: 0.02, dashArray: "8 5" },
  restaurations: { stroke: "#166534", fill: "#86efac", weight: 1.2, opacity: 0.9, fillOpacity: 0.28, dashArray: "" },
};
...
function featureIdentity(key: BoundaryKey, feature: any, index: number) { const p = feature.properties || {}; if (key === "bassins") return { id: String(p.Num_bassin ?? p.nom ?? index + 1), label: String(p.Nom ?? p.nom ?? `Bassin ${p.Num_bassin ?? index + 1}`) }; if (key === "sousBassins") return { id: String(p.HydroID ?? p.OBJECTID ?? index + 1), label: String(p.Nom ?? p.nom ?? `Sous-bassin ${p.HydroID ?? p.OBJECTID ?? index + 1}`) }; if (key === "communes") return { id: String(p.code ?? index + 1), label: String(p.commune ?? `Commune ${index + 1}`) }; return { id: String(p.id ?? index + 1), label: String(`${p.technologie ?? "Restauration"} — ${p.village ?? p.commune ?? index + 1}`) }; }
...
export default function LeafletMap({ module }: { module?: ModuleKey }) {
  ...
  const piezoRefLayer = useRef<any>(null);
  ...
  const [boundaryEnabled, setBoundaryEnabled] = useState<Record<BoundaryKey, boolean>>({ bassins: true, sousBassins: false, communes: false, restaurations: false });
  const [showReferencePiezos, setShowReferencePiezos] = useState(false);
  const [impactStats, setImpactStats] = useState<{sites:number;surface:number;technologies:Record<string,number>}>({sites:0,surface:0,technologies:{}});
  ...
  function applySelection(next: SpatialSelection) { setSelection(next); selectionRef.current = next; const points = next ? selectedPoints(allPointsRef.current) : allPointsRef.current; renderPoints(points); computeImpactStats(next); const detail = next ? { layer: next.layer, id: next.id, label: next.label } : null; try { if (detail) localStorage.setItem("psore-spatial-filter", JSON.stringify(detail)); else localStorage.removeItem("psore-spatial-filter"); } catch {} window.dispatchEvent(new CustomEvent("psore:spatial-filter", { detail })); }


  async function computeImpactStats(zone: SpatialSelection) {
    try {
      let data = boundaryDataRef.current.restaurations;
      if (!data) { const r = await fetch(layerUrls.restaurations); data = await r.json(); boundaryDataRef.current.restaurations = data; }
      const features = (data?.features || []).filter((f:any) => !zone || geometryContains(zone.geometry, centroidOf(f.geometry)));
      const technologies: Record<string,number> = {}; let surface = 0;
      features.forEach((f:any) => { const p=f.properties||{}; surface += Number(p.surface_ha||0); const t=String(p.technologie||"Non renseignée"); technologies[t]=(technologies[t]||0)+1; });
      setImpactStats({ sites: features.length, surface: Math.round(surface*100)/100, technologies });
    } catch { setImpactStats({sites:0,surface:0,technologies:{}}); }
  }
  function centroidOf(geometry:any): [number,number] { const coords:any[]=[]; const collect=(v:any)=>{ if(Array.isArray(v)&&typeof v[0]==="number") coords.push(v); else if(Array.isArray(v)) v.forEach(collect); }; collect(geometry?.coordinates); if(!coords.length) return [0,0]; return [coords.reduce((a,c)=>a+Number(c[0]),0)/coords.length, coords.reduce((a,c)=>a+Number(c[1]),0)/coords.length]; }
  ...
  useEffect(() => {
    async function syncReferencePiezos() {
      if (!window.L || !mapRef.current) return; const map=mapRef.current, L=window.L;
      if (!showReferencePiezos) { if (piezoRefLayer.current && map.hasLayer(piezoRefLayer.current)) map.removeLayer(piezoRefLayer.current); return; }
      if (piezoRefLayer.current) { piezoRefLayer.current.addTo(map); return; }
      const r=await fetch('/data/decisionnel/piezometres_ptcs.geojson'); const data=await r.json();
      const layer=L.geoJSON(data,{ pointToLayer:(_f:any,ll:any)=>L.circleMarker(ll,{radius:7,color:'#fff',weight:2,fillColor:'#dc2626',fillOpacity:.95}), onEachFeature:(f:any,l:any)=>{const p=f.properties||{}; l.bindPopup(`<div class="spatial-popup"><h4>Piézomètre PTCS ${safe(p.id)}</h4><table>${attributeRows(p,publicView)}</table></div>`);} });
      layer.addTo(map); piezoRefLayer.current=layer;
    } syncReferencePiezos().catch(()=>{});
  }, [showReferencePiezos, publicView]);
  ...
  return <div>
    ...
    <section className="boundary-panel">
      <div className="boundary-panel-header"><div><strong>Couches territoriales</strong><small>Bassins, sous-bassins, communes Enabel et restaurations</small></div><button className="btn btn-soft" onClick={() => setStyleOpen((v) => !v)}>{styleOpen ? "Fermer la palette" : "Palette des styles"}</button></div>
      <div className="map-tools boundary-toggle-tools">{(Object.keys(layerLabels) as BoundaryKey[]).map((key) => <button key={key} className={boundaryEnabled[key] ? "btn btn-primary" : "btn btn-soft"} onClick={() => setBoundaryEnabled((current) => ({ ...current, [key]: !current[key] }))}>{boundaryEnabled[key] ? "✓ " : ""}{layerLabels[key]}</button>)}<button className={showReferencePiezos ? "btn btn-primary" : "btn btn-soft"} onClick={() => setShowReferencePiezos(v => !v)}>{showReferencePiezos ? "✓ " : ""}Piézomètres PTCS (SHP)</button><button className="btn btn-soft" onClick={() => setBoundaryEnabled({ bassins: true, sousBassins: true, communes: true, restaurations: true })}>Tout afficher</button><button className="btn btn-soft" onClick={() => setBoundaryEnabled({ bassins: false, sousBassins: false, communes: false, restaurations: false })}>Tout masquer</button><button className="btn btn-soft" onClick={zoomToBoundaries}>Zoom sur les couches</button></div>
    ...
    <section className="sig-impact-panel"><div><strong>Analyse d’impact des restaurations</strong><small>{selection ? `Zone analysée : ${selection.label}` : "Ensemble de la zone du projet"}</small></div><div className="sig-impact-kpis"><span><b>{impactStats.sites}</b> sites restaurés</span><span><b>{impactStats.surface.toLocaleString("fr-FR")}</b> ha restaurés</span><span><b>{Object.keys(impactStats.technologies).length}</b> technologies</span></div><p>Base prête pour les analyses avant/après, les zones tampons autour des restaurations et la corrélation pluie–niveau piézométrique. Les résultats causaux nécessiteront des séries temporelles suffisantes et une date fiable de réalisation.</p></section>
