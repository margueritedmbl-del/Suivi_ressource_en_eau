...
export default function LeafletMap({ module }: { module?: ModuleKey }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        try { const br=await fetch("/data/admin-boundaries.geojson",{cache:"no-store"}); if(br.ok){ const b=await br.json(); if(!cancelled) L.geoJSON(b, { style: (f: any) => ({ color: f.properties.level === "Pays" ? "#5B2A86" : "#2D6A4F", weight: 2, fillOpacity: .05 }), onEachFeature: (f: any, l: any) => l.bindPopup(`${safe(f.properties.level)} : ${safe(f.properties.name)}`) }).addTo(map); } } catch {}
  ...
  useEffect(() => {
    async function loadPoints() {
      ...
      catch(e:any){ setSummary({}); setStatus(`Données cartographiques indisponibles : ${e?.message || "erreur"}`); return; }
