...
## 5.2.2 — Charte de reporting consolidée
- Ajout d'un **Brief collectivités** PDF d'une page.
- Rapport technique ramené à une structure principale de 3 pages, avec synthèse par station au lieu du tableau brut dans le PDF.
- Graphique pluviométrique principal remplacé par un classement des **cumuls par station**.
- Ajout d'une synthèse par commune avec couverture et cumul médian des stations.
- Ajout d'un graphique temporel de médiane du réseau, avec maximum journalier signalé.
- Les KPI et graphiques officiels utilisent uniquement les lignes classées `Validé` ; anomalies et doublons potentiels restent conservés et signalés.
- Paramétrage centralisé du jour de pluie dans `system_settings` avec référence MALI-METEO.
- Carte des rapports maintenue sur OSM avec emprise dynamique et centrage sur le réseau.


## V5.2.2 — Correctif reporting collectivités (09/09/2026)
- Correction de la cartographie des briefs : points conservés, coordonnées identiques décalées graphiquement, emprise dynamique et étiquettes limitées pour éviter les superpositions.
- Ajout du filtre Commune pour les rapports Points d’eau, aligné sur le Dashboard.
- Ajout d’une synthèse communale dédiée dans le brief Points d’eau.
