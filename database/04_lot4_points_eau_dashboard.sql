-- PSORE V2_4 LOT4 — Module Points d'eau décisionnel
-- À exécuter après database/00_migration_complete_v2_4.sql si la migration complète a déjà été lancée.
...
alter table public.points_eau add column if not exists enqueteur_initial text;
alter table public.points_eau add column if not exists date_collecte date;
alter table public.points_eau add column if not exists heure_collecte text;
...
create index if not exists idx_points_eau_priorite_gps on public.points_eau(latitude, longitude);
create index if not exists idx_points_eau_source_entry_id on public.points_eau(source_entry_id);



-- Vues décisionnelles Points d eau et cartographie
